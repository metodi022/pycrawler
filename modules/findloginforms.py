...
class FindLoginForms(Module):
    ...
    def add_handlers(self, browser: Browser, context: BrowserContext, page: Page,
        ...
        context_database.add_url((url_origin + '/login/', Config.DEPTH, self.crawler.rank, []))
        context_database.add_url((url_origin + '/signin/', Config.DEPTH, self.crawler.rank, []))
    ...
    def receive_response(self, browser: Browser, context: BrowserContext, page: Page,
        ...
        if response is None or response.status >= 400:
            ...
            if self._found <= 0 and len(context_database) <= 0 and Config.RECURSIVE:
                context_database.add_url(('https://www.google.com/search?q=' + urllib.parse.quote(f"\"login\" site:{self.crawler.site}"), Config.DEPTH - 1, self.crawler.rank, []))
            return
        ...
        if form is not None:
            ...
            self._found += 1
            self.crawler.state['FindLoginForms'] = self._found
            LoginForm.create(rank=self.crawler.rank, job=self.crawler.job_id,
