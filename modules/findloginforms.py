...
class FindLoginForms(Module):
    ...
    def add_handlers(self, url: Tuple[str, int, int, List[Tuple[str, str]]]) -> None:
        ...
        # self.crawler.context_database.add_url((self.crawler.site + '/signin/', Config.DEPTH, self.crawler.rank, []))
    ...
    def receive_response(self, responses: List[Optional[Response]],
        ...
        if form is not None:
            ...
            self._found += 1
            self.crawler.state['FindLoginForms'] = self._found
            LoginForm.create(job=self.crawler.job_id, crawler=self.crawler.crawler_id,
