...
class FindRegistrationForms(Module):
    ...
    def add_handlers(self, url: Tuple[str, int, int, List[Tuple[str, str]]]) -> None:
        ...
        self.crawler.urldb.add_url((self.crawler.origin + '/register/', Config.DEPTH, self.crawler.rank, []))
        self.crawler.urldb.add_url((self.crawler.origin + '/registration/', Config.DEPTH, self.crawler.rank, []))
        self.crawler.urldb.add_url((self.crawler.origin + '/signup/', Config.DEPTH, self.crawler.rank, []))
    ...
    def receive_response(self, responses: List[Optional[Response]],
        ...
        self.crawler.urldb.add_url(('https://www.google.com/search?q=' + urllib.parse.quote(f"\"register\" site:{self.crawler.site}"), Config.DEPTH - 1, self.crawler.rank, []))
